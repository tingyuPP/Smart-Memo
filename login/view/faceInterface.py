...
from qfluentwidgets import (SubtitleLabel, BodyLabel, PrimaryPushButton, 
                            TransparentPushButton, StateToolTip, FluentIcon as FIF,
                            InfoBar, InfoBarPosition, isDarkTheme, setTheme, Theme)
...
class FaceLoginInterface(QWidget):
    """人脸登录界面"""
    # 定义登录成功信号
    loginSuccessful = pyqtSignal(int, str)  # 用户ID和用户名
    ...
    def emit_login_signal(self):  
        """发送登录成功信号"""
        self.loginSuccessful.emit(self.verified_user_id, self.verified_username)
